...
export function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      if (key === 'pretty' || key === 'help' || key === 'version' || key === 'table' || key === 'no-retry' || key === 'cache' || key === 'no-cache' || key === 'stream' || key === 'enrich' || key === 'full' || key === 'unsafe-no-password' || key === 'no-password' || key === 'generate-password') {
